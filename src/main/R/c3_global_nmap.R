...
layitout <- function( g, ... ) {
    ut$layout_with_fr( g, niter=10000 )
...
plotit <- function( g, lo,
    ...
    clusters=FALSE, ealpha=.05, vcex=.4, vlabels.cex=.3, raster.edges=TRUE, ...
) {
    ut$plot_graph(
...
ut$gr_setup( n=4, device=DEVICE, file=( fig <- sprintf( '%s_ppmi', PROJ_NAME ) ) )
par( mar=c( 0,0,0,0 ) )
...
ut$gr_setup( n=4, device=DEVICE, file=( fig <- sprintf( '%s_difw', PROJ_NAME ) ) )
par( mar=c( 0,0,0,0 ) )
