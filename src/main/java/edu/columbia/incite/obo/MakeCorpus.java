...
public class MakeCorpus {
    ...
    public static Query epoch( int t0, int t1 ) {
        ...
        return new AutomatonQuery(
            new Term( OBODocFields.OBO_YEAR_FIELD, "" ),
    ...
    public static void main( String[] args ) throws IOException {
        ...
        Logs.infof( "====== Building global universe corpus ======" );
        ds = OBOSamples.getSample( obo.indexReader(), OBOSamples.UNIVERSE );
        ...
        Logs.infof( "====== Building global testimony corpus ======" );
        obo.conf().set( Conf.PARAM_DATA_DIR, root.toString() );
        ds = OBOSamples.getSample( obo.indexReader(), OBOSamples.TESTIMONY );
        obo.dumpFrequencies( obo.countFrequencies( ds ) );
        obo.dumpPOSCounts( obo.countPOSTags( ds ) );
        obo.dumpCooccurrences( obo.countCooccurrences( ds ) );

        Logs.infof( "====== Building epoch testimony corpora ======" );
        ...
        for( String epoch : EPOCHS.keySet() ) {
            ...
            Query q = ( new BooleanQuery.Builder() ).add(OBOSamples.TESTIMONY, Occur.FILTER
            ).add(
